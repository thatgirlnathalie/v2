...
function show() {
  ...
  hidetags.each(function (i) {
    ...
    if (height > 0 && height < 99999) {
      if (scrollpos + $(window).height() > $(this).offset().top + height) {
        count++;
        setTimeout(
          function (dis) {
  ...
  if (plax.length > 0) {
    if (scrollpos + $(window).height() > plax.offset().top && scrollpos + navh < plax.offset().top + plax.height()) {
      plax.css("background-position", "center " + ~~((-(plax.height() / 2) + (scrollpos - plax.offset().top + navh)) / 2) + "px");
    }
  }
} // end of show()
